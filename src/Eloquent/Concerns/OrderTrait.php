...
trait OrderTrait
{
        ...
        return Admin::cache('order.options.deliveries', function(){
            return Admin::getModel('Delivery')
                            ...
                            ->select(array_filter([
                                ...
                                'deliveries.name',
                                'deliveries.price',
                                config('admin_eshop.delivery.multiple_locations.enabled') ? 'deliveries.multiple_locations' : null,
                                'vats.vat'
                            ]))
                            ->get();
        });
    ...
    protected function getClientName()
    {
        ...
        if ( $this->client_id && $this->client ){
            return '<i class="fa fa-'.($this->client->isCompany ? 'building' : 'user').' mr-1" data-toggle="tooltip" title="'._('Klient č.').' '.implode(' / ', array_filter([$this->client_id, $this->client->clientName])).'"></i> '.$clientName;
        }
    ...
    public function getVerifiedCustomersItemsIds()
    {
        return $this->items->map(function($item){
            ...
            if ( method_exists($product, 'getHeurekaItemIdAttribute') ) {
